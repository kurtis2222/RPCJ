...
public class HeadBob : MonoBehaviour {
	...
	void FixedUpdate()
	{
		...
		{
			transform.localPosition = Vector3.Slerp(transform.localPosition,def_pos,Time.fixedDeltaTime*20);
	...
	void DoBob()
	{
		...
		if(tmpy > limits[0] && !isdown) isdown = true;
		else if(tmpy < limits[1] && isdown) isdown = false;
	}
}