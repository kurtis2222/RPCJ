...
public class FPSWalkerEnhanced: MonoBehaviour {
	...
	public bool enable_crouch = true;
	public bool enable_run = true;
	public bool enable_fdam = false;
    ...
    void FixedUpdate()
	{
		...
		if(can_control)
		{
		    ...
		    if (grounded) {
		        ...
		        if (falling) {
		            falling = false;
		            if (enable_fdam && myTransform.position.y < fallStartLevel - fallingDamageThreshold)
		                FallingDamageAlert (fallStartLevel - myTransform.position.y);
		        }
		
		        // If running isn't on a toggle, then use the appropriate speed depending on whether the run button is down
		        if (!toggleRun)
		            speed = enable_run && Input.GetButton("Run/Sprint")? runSpeed : walkSpeed;
				...
				if (!Input.GetButton("Jump"))
		            jumpTimer++;
		        else if (enable_jump && jumpTimer >= antiBunnyHopFactor) {
			...
			if(isinwater)
			{
				moveDirection = maincam.transform.forward * inputY * speed * inputModifyFactor;
				moveDirection += myTransform.right * inputX * speed * inputModifyFactor;
			}
		}
	...
	public void Jumppad()
	{
		moveDirection = new Vector3(0f,30f,0f);
		grounded = (controller.Move(moveDirection * Time.deltaTime) & CollisionFlags.Below) != 0;
	}
}